...
> 💾 이 둘은 **휴대폰 안에 저장**됩니다. 서버 설정이 전혀 필요 없어요.
> (브라우저 데이터를 지우면 함께 사라집니다. 다시 적으면 됩니다)
...
**Vercel 서버는 파일을 쓸 수 없습니다** (읽기 전용). 그래서 저장 위치를 나눴어요.
...
| 📋 네이버·헬로톡 글 기록 | **휴대폰** | ✅ 정상 |
| 🔒 로그인 상태 | 쿠키 | ✅ 정상 |
| ▶️ 유튜브 계정 연결 | 서버 파일 | ❌ 저장 불가 |
| 📸 인스타 계정 연결 | 서버 파일 | ❌ 저장 불가 |
| 📎 네이버·헬로톡에 넣을 사진 | **휴대폰** | ✅ 정상 |
| 📎 유튜브·인스타에 올릴 파일 | 서버 파일 | ❌ 저장 불가 |
| ⏰ 예약 발행 | 서버 파일 | ❌ 저장 불가 |

### 그래서 Vercel에서는

- ✅ **네이버 · 헬로톡은 지금 바로 완벽하게 됩니다**
- ⚠️ 유튜브 · 인스타는 **저장소를 따로 붙여야** 씁니다

앱의 **[⚙️ 설정]** 탭에 들어가면 지금 내 서버가 저장 가능한지 **자동으로 알려줍니다.**

### 유튜브·인스타까지 쓰고 싶다면

**둘 중 하나**를 고르면 됩니다.

1. **디스크를 주는 곳에 올리기** — [Railway](https://railway.app), [Render](https://render.com), 집 컴퓨터 등
   → 코드 수정 없이 지금 그대로 전부 동작합니다
2. **Vercel에 저장소 붙이기** — Vercel Blob(파일) + Upstash Redis(설정) 같은 무료 서비스
   → 코드 수정이 조금 필요합니다
...
src/
 ...
 └ lib/
    ...
    ├ localStore.ts         휴대폰 안에 저장 (네이버·헬로톡·글기록)
    └ publish.ts            전체 지휘
```
